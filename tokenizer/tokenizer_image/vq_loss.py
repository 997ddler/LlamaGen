...
class VQLoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=256,
        ...
        ## logger
        if tdist.get_rank() == 0:
            self.wandb_tracker = wandb.init(project='LlamaGen-VQ',)
    ...
    def validation(self, inputs, reconstructions):
            # reconstruction loss
            rec_loss = self.rec_loss(inputs.contiguous(), reconstructions.contiguous())
            ...
            rec_p_loss = self.rec_weight * rec_loss + self.perceptual_weight * p_loss
            
            return rec_loss, p_loss, rec_p_loss